...
// Obliga a escribir Color::AZUL en lugar de solo AZUL,
// evitando conflictos entre enums distintos que tengan el mismo nombre
enum class Color { AZUL, CELESTE, ROJO, AMARILLO };
enum class Equipo { JUGADOR1, JUGADOR2 };
enum class Direccion { NORTE, SUR, ESTE, OESTE };


// inline le dice al compilador que copie el cuerpo de la funcion donde se llama,
// evitando el costo de un salto de funcion
// Convierte un Color a texto
...
class Tank {
    int x;
    int y;
    int vida;            // Vida actual, disminuye al recibir dano
    int vidaMax;         // Vida maxima original, referencia fija para calcular porcentajes y dano total
    const Equipo equipo;
    const Color color;
    Direccion direccion;
...
public:
    ...
    //Actualiza x, y y calcula automaticamente la nueva direccion
